...
<template>
  ...
  <div class="min-h-full max-w-5xl mx-auto w-full p-8 transition-colors duration-300 space-y-8">
    ...
    <!-- Profile Section -->
    <section class="p-6 rounded-xl bg-(--bg2) border border-(--white)/10">
        <h2 class="font-semibold text-lg mb-4 flex items-center gap-2">
            <i class="bi bi-person-fill text-(--btn)" /> Profil utilisateur
        </h2>
        <UserProfile :fallback="Loader" />
        <i class="text-(--text-little) text-sm">User id : {{ useUser().user.value?.id }}</i>
    </section>

    <!-- AI Features Section -->
    <section class="p-6 rounded-xl bg-(--bg2) border border-(--white)/10">
        <h2 class="font-semibold text-lg mb-4 flex items-center gap-2">
            <i class="bi bi-robot text-(--btn)" /> Agent IA : SilverIA
        </h2>
        
        <div class="p-6 rounded-xl bg-(--bg) border border-(--white)/5 transition-all" :class="!aiFunc ? 'opacity-75' : ''">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 transition-all" :class="!aiFunc ? 'pointer-events-none opacity-20' : ''">
                <label class="flex items-center gap-3 p-3 rounded-lg bg-(--bg2)/50 border border-(--white)/5 pointer-events-none">
    ...
    <!-- Encryption Section -->
    <section class="p-6 rounded-xl bg-(--bg2) border border-(--white)/10 overflow-hidden relative">
